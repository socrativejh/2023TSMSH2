...
def requests_call(input_file_name):
    ...
    url = 'https://app.nanonets.com/api/v2/OCR/Model/c02fac3f-6574-42e3-b2e4-6552358959e7/LabelFile/'

    ############## 수정 ##########################
    file_path = 'apps/static/files/storage/'+ input_file_name # request 받은 파일 저장하는 폴더명 수정
    ##############################################

    file_name = input_file_name.split(".")[0]
    
    ############## 수정 ##########################
    result_file_path = "apps/static/files/nanonets/" + file_name +".json" # nanonets 결과물 저장되어 있는 폴더 경로 수정
    ##############################################
    ...
    if(os.path.isfile(result_file_path) == False):  
        ...
        response = requests.post(url, auth=requests.auth.HTTPBasicAuth('36f6aaaa-9626-11ed-8308-d6abab017d2a', ''), files=data)
    ...
    ############## 수정 ##########################
    insert_file_path = os.path.join(script_dir, f"apps/static/files/result/{file_name}.json") # 가공된 결과물 저장하기 위한 파일 경로 수정
    ##############################################

    with open(insert_file_path, 'w',encoding='utf-8') as json_file:
